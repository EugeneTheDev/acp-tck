"""Meta-tests over `tck.requirements.REGISTRY`: shape invariants, plus a two-way check against
every `@pytest.mark.requirement(...)` used under `src/tck/conformance/` (import the modules and
inspect `pytestmark` directly -- no grep, per the task spec).
"""
...
def _requirement_ids_used_in_conformance_tests() -> set[str]:
    import tck.conformance as conformance_pkg
