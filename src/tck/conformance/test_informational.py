...
"Open questions"), so none of these ever assert on the probed behaviour itself -- they exist
purely to record what the agent under test actually does, via `record_property`, for a human
reading the report. (They can still FAIL if the prerequisite handshake -- `initialize`, or
`session/new` for ACP-INFO-UNKNOWNSESSION-001 -- itself fails; that is a real conformance
problem the probe correctly surfaces, not a probe bug.) Each uses its own throwaway
`connected_agent` so a bad reaction (if any) cannot contaminate a later test's connection.
...
(`.agents/research/review-slices-7.md` S2).
"""
...
async def test_malformed_json_line_behaviour(agent_launch, tmp_path, record_property):
    ...
    documented-but-not-universal behaviour), replies with something else, or stays silent (the
    reference SDK's documented behaviour -- concluded via `quiet_period()`, not the full
    `--tck-timeout`, review-slices-7.md S2). Then, using a fresh, ordinary `session/new` on the
    *same* connection, records whether the connection is still usable afterwards. Never asserts
    on the probed behaviour itself."""
...
async def test_structurally_invalid_request_behaviour(agent_launch, tmp_path, record_property):
    ...
    *is* valid JSON yet not a JSON-RPC request/notification/response at all (`{"foo": "bar"}`,
    no `jsonrpc`/`method`/`id`). "Silent" is concluded via `quiet_period()`, not the full
    `--tck-timeout` (review-slices-7.md S2). Never asserts on the probed behaviour itself."""
    async with connected_agent(agent_launch, handshake=False) as agent:
        init_id = await agent.send_request(
            "initialize", {"protocolVersion": PROTOCOL_VERSION, "clientCapabilities": {}}
        )
...
async def test_unknown_session_id_behaviour(agent_launch, tmp_path, record_property):
    ...
    async with connected_agent(agent_launch) as agent:
        ...
        try:
            ...
            # N13 (review-slices-7.md): a bare `wait_for_response` has no mock client, so an
            # agent that issues e.g. `session/request_permission` while handling the bogus
            # session will never be answered and this looks like plain silence unless we say
            # otherwise -- note any outstanding agent -> client request left unanswered.
            outstanding = [
                entry.parsed.get("method")
                for entry in agent.pending()
                if isinstance(entry.parsed, dict) and entry.parsed.get("method") and "id" in entry.parsed
            ]
            if outstanding:
                behaviour = f"silent (agent had {len(outstanding)} outstanding client request(s): {outstanding})"
            else:
                behaviour = "silent"
        except AgentExited as exc:
            behaviour = f"agent exited (exit_code={exc.exit_code!r})"
