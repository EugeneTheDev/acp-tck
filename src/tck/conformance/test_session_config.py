...
`"inferred:configOptions"` string (see `tck.requirements` module docstring for why this is the
smallest change that satisfies `Requirement.__post_init__`'s invariant) -- it is *not* looked up
by `tck.plugin`'s `@pytest.mark.capability(...)` marker/`_tck_capability_gate` fixture, since
