...
- **Copy commands used:**
  ...
  ```
  cp <spec-repo>/schema/v1/schema.json src/tck/schema/v1/schema.json
  cp <spec-repo>/schema/v1/meta.json   src/tck/schema/v1/meta.json
  ```
...
4. Re-run `uv run pytest` — `tests/test_validation.py` will catch any shape the new schema
   changed that `src/tck/protocol.py` / `src/tck/validation.py` assumed.
5. If method names, `x-side`/`x-method` annotations, or the top-level `anyOf` branch layout
   (`Agent` / `Client` / `ProtocolLevel`) changed shape, re-check `src/tck/protocol.py` and
   `src/tck/validation.py` by hand — both parse the schema structurally, not just by value.
