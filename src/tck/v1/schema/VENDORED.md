...
`schema.json` and `meta.json` in this directory are verbatim copies of the ACP v1 JSON Schema,
...
2. Re-run the two commands above with the new checkout path.
3. Update the commit hash and date in this file, and `SCHEMA_REVISION` in
   `src/tck/v1/protocol.py`.
