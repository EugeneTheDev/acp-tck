...
A separate module from `test_transport.py`/`test_initialize.py` to keep the "jsonrpc:" group of
assertions in one focused file. See `AGENTS.md` "How to add a requirement + test".
...
async def test_id_is_echoed_for_integer_and_string_ids(agent_launch, tmp_path):
    ...
    The integer-id half uses `initialize` itself (a request every agent MUST answer); the
    string-id half hand-rolls its own `session/new` request rather than going through
    `new_session`/`skip_if_auth_gated` -- id echo holds even for a `-32000` (auth-required)
    reply, so there is nothing here for an auth-gated agent to break."""
...
async def test_response_envelope_is_result_xor_error_with_valid_shape(agent_launch):
    ...
    to that at all is only SHOULD (see ACP-JSONRPC-004). This does not assert which error code
    either response uses, only that the envelope (`id`, `result` xor `error`, error shape) is
    well-formed.
    """
    async with connected_agent(agent_launch, handshake=False) as agent:
...
async def test_unknown_method_yields_method_not_found(agent_launch):
    ...
    instead of failing it -- whether the agent responds *at all* to an unrecognised method is a
    separate, stronger concern for `_`-prefixed custom methods specifically (Req
...
async def test_notification_receives_no_response(agent_launch, tmp_path):
    ...
    Also covers a would-be `ACP-CANCEL-003` ("`session/cancel` itself is a notification and
    receives no response", J2 applied to cancel): no separate id was registered for that since
    it is exactly what this test already asserts.
    ...
    async with connected_agent(agent_launch) as agent:
        ...
        def _is_a_response(entry) -> bool:
            ...
            # as a response candidate rather than requiring `id` to be present.
...
async def test_connection_survives_an_erroneous_request(agent_launch, tmp_path):
    ...
    a timeout is suppressed rather than propagated -- an unguarded `wait_for_response` would
    fold a legal silence into a FAIL and cost a full `--timeout` of dead wall-clock doing it.
