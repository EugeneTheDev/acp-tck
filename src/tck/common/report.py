...
`tck.common.plugin` is the only producer (it collects `TestOutcome`s from pytest hooks and
builds a `Report` at `pytest_sessionfinish`); this module has no pytest dependency so it can be
unit tested with synthetic data (`tests/common/test_report.py`). Version-agnostic: it carries no
...
from .requirements import Requirement, Tier
...
def build_requirement_results(
    tests_by_id: dict[str, list[TestOutcome]], registry: dict[str, Requirement]
